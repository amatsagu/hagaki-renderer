...
pub async fn handle_card_fan_request(Path(hash): Path<String>, Extension(frames): Extension<Arc<RwLock<HashMap<u32, DynamicImage>>>>) -> Response<Body> {
    ...
    let image = match render_fan(decoded.cards, &frames_guard, &start) {
        Ok(image) => image,
